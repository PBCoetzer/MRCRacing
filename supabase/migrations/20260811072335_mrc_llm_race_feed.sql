...
begin
  ...
  set
    ...
    completed_at = now(),
    duration_ms = greatest(0, floor(extract(epoch from now()) * 1000)::bigint - started_milliseconds)::integer,
    meetings_seen = apply_race_snapshot.meetings_seen,
    races_seen = apply_race_snapshot.races_seen,
    entries_seen = apply_race_snapshot.entries_seen,
    changes_applied = apply_race_snapshot.changes_applied,
    alerts_created = apply_race_snapshot.alerts_created,
    extracted_payload = target_snapshot,
    error_code = null,
    error_message = null
  where id = target_run_id;
...
create policy tip_card_change_alerts_admin_read
...
create policy tip_card_change_alerts_owner_read
on public.tip_card_change_alerts
for select
to authenticated
using (
